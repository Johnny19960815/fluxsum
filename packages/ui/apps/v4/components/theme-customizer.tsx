...
function CustomizerCode({ themeName }: { themeName: string }) {
  ...
  const activeTheme = React.useMemo(
    () => baseColors.find((theme: BaseColor) => theme.name === themeName),
    [themeName]
  )
          ...
          >
            ...
            <pre className="no-scrollbar max-h-[300px] min-w-0 overflow-x-auto px-4 py-3.5 outline-none has-[[data-highlighted-line]]:px-0 has-[[data-line-numbers]]:px-0 has-[[data-slot=tabs]]:p-0 md:max-h-[450px]">
              ...
              <code data-line-numbers data-language="css">
                ...
                {Object.entries(activeThemeOKLCH?.light ?? {} as Record<string, string>).map(([key, value]: [string, string]) => (
                  <span
                    data-line
                ...
                {Object.entries(activeThemeOKLCH?.dark ?? {} as Record<string, string>).map(([key, value]: [string, string]) => (
                  <span
                    data-line
                    className="line text-code-foreground"
                    key={key}
          ...
          >
            ...
            <pre className="no-scrollbar max-h-[300px] min-w-0 overflow-x-auto px-4 py-3.5 outline-none has-[[data-highlighted-line]]:px-0 has-[[data-line-numbers]]:px-0 has-[[data-slot=tabs]]:p-0 md:max-h-[450px]">
              ...
              <code data-line-numbers data-language="css">
                ...
                {Object.entries(activeTheme?.cssVars.light || {} as Record<string, string>).map(
                ...
                {Object.entries(activeTheme?.cssVars.dark || {} as Record<string, string>).map(
                  ([key, value]: [string, string]) => (
                    <span
                      data-line
                      className="line text-code-foreground"
