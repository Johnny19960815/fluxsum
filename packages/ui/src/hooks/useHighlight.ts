...
import lobeTheme from '@/Highlighter/theme/flxsum-theme';
  ...
  useEffect(() => {
    ...
    const highlightPromise = (async (): Promise<string> => {
      try {
        ...
        const effectiveTheme = builtinTheme || 'flxsum-theme';

        // Load custom theme if using slack-dark or slack-ochin
        if (!builtinTheme && effectiveTheme === 'flxsum-theme') {
