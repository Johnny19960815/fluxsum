...
import { GradientButton } from '@fluxsum/ui/awesome';
...
const PopoverContent = ({ title }: { title: string }) => (
  <Flexbox gap={8} style={{ padding: 12, width: 200 }}>
    ...
    <Text style={{ color: 'var(--flxsum-color-text-3)', fontSize: 13 }}>
...
const Section = ({
  ...
  <Flexbox gap={12}>
    <Flexbox gap={4}>
      ...
      <Text style={{ color: 'var(--flxsum-color-text-3)', fontSize: 13 }}>{description}</Text>
    </Flexbox>
  ...
  return (
    <Flexbox gap={32} style={{ padding: 24 }}>
      <Flexbox gap={8}>
        ...
        <Text style={{ color: 'var(--flxsum-color-text-3)', fontSize: 14 }}>
      ...
      >
        ...
        <Popover content={<PopoverContent title="Emoji Trigger" />} trigger="click">
          <span style={{ cursor: 'pointer', fontSize: 32 }}>🎉</span>
        </Popover>
